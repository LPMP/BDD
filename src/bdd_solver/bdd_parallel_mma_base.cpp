...
namespace LPMP {
    ...
    template<typename BDD_BRANCH_NODE>
        bdd_parallel_mma_base<BDD_BRANCH_NODE>::bdd_parallel_mma_base(const BDD::bdd_collection& bdd_col)
        {
            add_bdds(bdd_col);
        }

    template<typename BDD_BRANCH_NODE>
        bdd_parallel_mma_base<BDD_BRANCH_NODE>::bdd_parallel_mma_base(const BDD::bdd_collection& bdd_col, const std::vector<double>& costs_hi)
        : bdd_parallel_mma_base(bdd_col)
        {
    ...
    template<typename BDD_BRANCH_NODE>
        void bdd_parallel_mma_base<BDD_BRANCH_NODE>::add_bdds(const BDD::bdd_collection& bdd_col)
        {
            ...
            bdd_log << "[bdd parallel mma base] # bdds = " << bdd_col.nr_bdds() << "\n";
            ...
            bdd_log << "[bdd parallel mma base] # total bdd nodes = " << total_nr_bdd_nodes << "\n";
            ...
            bdd_log << "[bdd parallel mma base] # vars = " << nr_vars << "\n";
...
//        }
    ...
    template<typename BDD_BRANCH_NODE>
        void bdd_parallel_mma_base<BDD_BRANCH_NODE>::update_costs(const std::vector<value_type>& costs_lo, const std::vector<value_type>& costs_hi)
        {
            update_costs(costs_lo.begin(), costs_hi.begin(), costs_lo.end(), costs_hi.end());
        }
...
//        }
    ...
    template<typename BDD_BRANCH_NODE>
        void bdd_parallel_mma_base<BDD_BRANCH_NODE>::fix_variables(const std::vector<size_t> zero_vars, const std::vector<size_t> one_vars)
        {
            fix_variables(zero_vars.begin(), zero_vars.end(), one_vars.begin(), one_vars.end());
        }
...
#pragma omp parallel for schedule(dynamic) reduction(+:lb)
    ...
    template<typename BDD_BRANCH_NODE>
        void bdd_parallel_mma_base<BDD_BRANCH_NODE>::iteration()
        {
            backward_run();
...
#pragma omp parallel for
    ...
    template<typename BDD_BRANCH_NODE>
        ...
        template <typename BDD_BRANCH_NODE>
        std::vector<char> bdd_parallel_mma_base<BDD_BRANCH_NODE>::bdds_solution_vec()
        {
            backward_run();
            ...
            assert(solutions.size() == nr_bdd_variables());
        ...
        std::vector<typename BDD_BRANCH_NODE::value_type> bdd_parallel_mma_base<BDD_BRANCH_NODE>::net_solver_costs()
        {
            ...
            for(size_t bdd_nr=0; bdd_nr<nr_bdds(); ++bdd_nr)
            {
                ...
                {
                    const auto [first,last] = bdd_index_range(bdd_nr, idx);
                    value_type low_cost = -std::numeric_limits<value_type>::infinity();
                    value_type high_cost = -std::numeric_limits<value_type>::infinity(); 
                    for(size_t i=first; i<last; ++i)
                    {
                        const auto& bdd = bdd_branch_nodes_[i];
                        if(bdd.offset_low != BDD_BRANCH_NODE::terminal_0_offset)
                            low_cost = bdd.low_cost;
                        if(bdd.offset_high != BDD_BRANCH_NODE::terminal_0_offset)
                            high_cost = bdd.high_cost;
                    }
                    assert(low_cost !=  -std::numeric_limits<value_type>::infinity());
        ...
        template <typename BDD_BRANCH_NODE>
        bool bdd_parallel_mma_base<BDD_BRANCH_NODE>::dual_feasible(const std::vector<value_type>& duals) const
        {
            assert(duals.size() == nr_bdd_variables());
            std::vector<value_type> dual_sum_per_var(nr_variables(), 0.0);

            size_t c = 0;
            for (size_t bdd_nr = 0; bdd_nr < nr_bdds(); ++bdd_nr)
                for (size_t bdd_idx = 0; bdd_idx < nr_variables(bdd_nr); ++bdd_idx)
                    dual_sum_per_var[variable(bdd_nr, bdd_idx)] += duals[c++];
            assert(c == nr_bdd_variables());

            for (size_t i = 0; i < dual_sum_per_var.size(); ++i)
                if (std::abs(dual_sum_per_var[i]) >= 1e-6)
                    return false;
            return true;
        }

        template <typename BDD_BRANCH_NODE>
        void bdd_parallel_mma_base<BDD_BRANCH_NODE>::make_dual_feasible(std::vector<value_type>& duals) const
        {
            assert(duals.size() == nr_bdd_variables());
            std::vector<value_type> dual_sum_per_var(nr_variables(), 0.0);
            ...
            for(size_t i=0; i<dual_sum_per_var.size(); ++i)
                dual_sum_per_var[i] /= value_type(nr_bdds(i));

            c = 0;
            for (size_t bdd_nr = 0; bdd_nr < nr_bdds(); ++bdd_nr)
                for (size_t bdd_idx = 0; bdd_idx < nr_variables(bdd_nr); ++bdd_idx)
                    duals[c++] -= dual_sum_per_var[variable(bdd_nr, bdd_idx)];
            assert(c == nr_bdd_variables());
        }

        template <typename BDD_BRANCH_NODE>
        void bdd_parallel_mma_base<BDD_BRANCH_NODE>::gradient_step(const std::vector<value_type>& duals, const double step_size)
        {
            message_passing_state_ = message_passing_state::none;
            lower_bound_state_ = lower_bound_state::invalid; 

            //assert(step_size > 0.0); // TODO: correct
            assert(duals.size() == nr_bdd_variables());
            ...
            for (size_t bdd_nr = 0; bdd_nr < nr_bdds(); ++bdd_nr)
            {
                for (size_t bdd_idx = 0; bdd_idx < nr_variables(bdd_nr); ++bdd_idx, ++c)
                {
                    const auto [first, last] = bdd_index_range(bdd_nr, bdd_idx);
                    for (size_t i = first; i < last; ++i)
                    {
                        auto& bdd = bdd_branch_nodes_[i];
                        if(bdd.offset_high != BDD_BRANCH_NODE::terminal_0_offset)
                            bdd.high_cost += step_size * duals[c];
                    }
                }
            }
        ...
        template <typename BDD_BRANCH_NODE>
        size_t bdd_parallel_mma_base<BDD_BRANCH_NODE>::nr_layers() const
        {
            return nr_bdd_variables();
        }

        template class bdd_parallel_mma_base<bdd_branch_instruction<float, uint16_t>>;
        template class bdd_parallel_mma_base<bdd_branch_instruction<double, uint16_t>>;
        template class bdd_parallel_mma_base<bdd_branch_instruction_smooth<float, uint16_t>>;
        template class bdd_parallel_mma_base<bdd_branch_instruction_smooth<double, uint16_t>>;
}
