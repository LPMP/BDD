...
namespace LPMP {
    ...
    bdd_mma_base<BDD_BRANCH_NODE>::bdd_mma_base(const BDD::bdd_collection& bdd_col)
    {
        add_bdds(bdd_col);
    }

    template<typename BDD_BRANCH_NODE>
    bdd_mma_base<BDD_BRANCH_NODE>::bdd_mma_base(const BDD::bdd_collection& bdd_col, const std::vector<double>& costs_hi)
    : bdd_mma_base(bdd_col)
    {
    ...
    template<typename BDD_BRANCH_NODE>
        void bdd_mma_base<BDD_BRANCH_NODE>::update_costs(const std::vector<value_type>& costs_lo, const std::vector<value_type>& costs_hi)
        {
            update_costs(costs_lo.begin(), costs_lo.end(), costs_hi.begin(), costs_hi.end());
        }
    ...
    template<typename BDD_BRANCH_NODE>
        ...
        void bdd_mma_base<BDD_BRANCH_NODE>::update_costs(COST_ITERATOR cost_lo_begin, COST_ITERATOR cost_lo_end, COST_ITERATOR cost_hi_begin, COST_ITERATOR cost_hi_end)
        {
            ...
            auto get_cost = [](const size_t var, auto cost_begin, auto cost_end) -> value_type{
...
//        }
    ...
    std::vector<size_t> bdd_mma_base<BDD_BRANCH_NODE>::add_bdds(const BDD::bdd_collection &bdd_col, BDD_NR_ITERATOR bdd_nrs_begin, BDD_NR_ITERATOR bdd_nrs_end)
    {
        ...
        template <typename BDD_BRANCH_NODE>
        two_dim_variable_array<char> bdd_mma_base<BDD_BRANCH_NODE>::bdd_feasibility(const std::vector<char>& sol) const
        {
            return bdd_feasibility(sol.begin(), sol.end());
        }
    ...
    template class bdd_mma_base<bdd_branch_instruction_bdd_index<float, uint32_t>>;
    template class bdd_mma_base<bdd_branch_instruction_bdd_index<double, uint32_t>>;
    template class bdd_mma_base<bdd_branch_instruction_smooth_bdd_index<float, uint32_t>>;
    template class bdd_mma_base<bdd_branch_instruction_smooth_bdd_index<double, uint32_t>>;
}
