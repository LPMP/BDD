...
namespace LPMP {
    ...
    typename bdd_solver::solver_type bdd_solver::construct_solver(const nlohmann::json&config, const BDD::bdd_collection& bdd_col, const std::vector<double>& costs)
    {
        ...
        if (solver_type == "sequential mma")
        {
            if (precision == "double")
            {
                return bdd_mma_base<bdd_branch_instruction_bdd_index<double, uint32_t>>(bdd_col, costs.begin(), costs.end());
            }
            else
            {
                return bdd_mma_base<bdd_branch_instruction_bdd_index<float, uint32_t>>(bdd_col, costs.begin(), costs.end());
            }
        }
        else if (solver_type == "parallel mma")
        {
            if (precision == "double")
            {
                return bdd_parallel_mma_base<bdd_branch_instruction<double, uint16_t>>(bdd_col, costs.begin(), costs.end());
            }
            else
            {
                return bdd_parallel_mma_base<bdd_branch_instruction<float, uint16_t>>(bdd_col, costs.begin(), costs.end());
            }
        }
#ifdef WITH_CUDA
            else if(solver_type == "cuda parallel mma")
            {
                if (precision == "double")
                {
                    return bdd_cuda_parallel_mma<float>(bdd_col, costs.begin(), costs.end());
                }
                else
                {
                    return bdd_cuda_parallel_mma<double>(bdd_col, costs.begin(), costs.end());
                }
            }
#endif
            else if(solver_type.find("lbfgs") == 0)
            {
                ...
                if (precision == "double")
                {
                    return lbfgs<bdd_parallel_mma_base<bdd_branch_instruction<double, uint16_t>>, std::vector<double>, double, std::vector<char>>(bdd_col, costs.begin(), costs.end(),
                ...
                {
                    return lbfgs<bdd_parallel_mma_base<bdd_branch_instruction<float, uint16_t>>, std::vector<float>, float, std::vector<char>>(bdd_col, costs.begin(), costs.end(),
                    lbfgs_history_size, lbfgs_init_step_size, lbfgs_req_rel_lb_increase, lbfgs_step_size_decrease_factor, lbfgs_step_size_increase_factor);
                }
                }
                #ifdef WITH_CUDA
                else if(solver_type == "lbfgs cuda mma")
                {
                    if (precision == "double")
                    {
                        return lbfgs<bdd_cuda_parallel_mma<double>, thrust::device_vector<double>, double, thrust::device_vector<char>(bdd_col, costs.begin(), costs.end(),
                        lbfgs_history_size, lbfgs_init_step_size, lbfgs_req_rel_lb_increase, lbfgs_step_size_decrease_factor, lbfgs_step_size_increase_factor);
                    }
                    else
                    {
                        return lbfgs<bdd_cuda_parallel_mma<float>, thrust::device_vector<float>, float, thrust::device_vector<char>(bdd_col, costs.begin(), costs.end(),
                        lbfgs_history_size, lbfgs_init_step_size, lbfgs_req_rel_lb_increase, lbfgs_step_size_decrease_factor, lbfgs_step_size_increase_factor);
                    }
                }
                #endif
            ...
            else if(solver_type == "subgradient")
            {
                if (precision == "double")
                {
                    return subgradient<bdd_parallel_mma_base<bdd_branch_instruction<float, uint16_t>>, float>(bdd_col, costs.begin(), costs.end());
                }
                else
                {
                    return subgradient<bdd_parallel_mma_base<bdd_branch_instruction<double, uint16_t>>, double>(bdd_col, costs.begin(), costs.end());
                }
            }
#ifdef WITH_CUDA
        ...
        if (config.contains("perturbation rounding"))
        {
            ...
            const auto sol = std::visit([&](auto &&s) {
                if constexpr( // CPU rounding
                    ...
                    {
                    return s.incremental_mm_agreement_rounding(init_perturbation, perturbation_growth_rate, nr_inner_iterations, nr_outer_iterations);
                    }
#endif
                    else
                    {
