...
ILP_input multi_graph_matching_bdd_solver::read_ILP(const nlohmann::json &config)
{
    ...
    if (f)
    {
        ...
        return LPMP::parse_multi_graph_matching_file(input);
    }
    else // input might be ILP in string
    {
        bdd_log << "[graph matching solver] Read input string " << config["input"] << "\n";
        return LPMP::parse_multi_graph_matching_string(input);
    }
}
