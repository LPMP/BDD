...
PYBIND11_MODULE(bdd_cuda_parallel_mma_py, m) {
    ...
    py::class_<bdd_type>(m, "bdd_cuda_parallel_mma")
        ...
        .def("nr_variables", [](bdd_type& solver, const int block_idx) { return solver.nr_variables(block_idx); })
        .def("nr_variable_blocks", &bdd_type::nr_variable_blocks)
        .def("nr_bdds", &bdd_type::nr_bdds)
        .def("lower_bound", &bdd_type::lower_bound)
        .def("min_marginal_diff_of_block", [](bdd_type& solver, const int block_idx, const long mm_diff_out_ptr) 
        {
            ...
            float* const mm_diff_ptr = reinterpret_cast<float* const>(mm_diff_out_ptr); // Points to memory allocated by Python.
            solver.min_marginals_from_directional_costs(block_idx, 1.0, mm_diff_ptr);
        })
    ;
}
