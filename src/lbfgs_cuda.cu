...
namespace LPMP {
    ...
    void lbfgs_cuda<REAL>::store_next_itr(thrust::device_vector<REAL>& cur_x, thrust::device_vector<REAL>& cur_grad_f)
    {
        ...
        else
        {
            ...
            // compute grad_f_k - grad_f_{k-1}, but since we are cur_grad_f is for maximization problem and lbfgs updates are derived for minimization so multiply gradients by -1.
            ...
            if (rho_inv > 1e-3) // otherwise, skip the iterate as curvature condition is not strongly satisfied.
            {
    ...
    bool lbfgs_cuda<REAL>::project_gradient(thrust::device_ptr<REAL> grad_f)
    {
        ...
        for (int count = 0; count < num_history; count++)
        {
            ...
            const REAL alpha = thrust::inner_product(s_history[i].begin(), s_history[i].end(), grad_f, (REAL) 0.0) / (1e-6 + rho_inv_history[i]);
        ...
        REAL initial_H_diag_multiplier = rho_inv_history.back() / (1e-6 + last_y_norm);
        ...
        for (int count = 0; count < num_history; count++)
        {
            ...
            REAL current_rho = 1 / (1e-6 + rho_inv_history[i]);
            if (count == 0)
            ...
            update_r<REAL> update_r_func({alpha_history[count], beta, thrust::raw_pointer_cast(s_history[i].data()), thrust::raw_pointer_cast(grad_f)});
