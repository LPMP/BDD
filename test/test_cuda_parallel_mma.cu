...
void test_problem(const std::string& problem_input)
{
    ...
    // map variables to new ones
    {
        ...
        std::vector<size_t> var_map(1,distrib(gen));
        ...
        {
            std::vector<std::array<double,2>> cpu_delta_in(parallel_mma.nr_variables(), std::array<double,2>{0.0, 0.0});
            std::vector<std::array<double,2>> cpu_delta_out(parallel_mma.nr_variables(), std::array<double,2>{0.0, 0.0});
            thrust::device_vector<double> cuda_delta_lo(parallel_mma.nr_variables(), 0.0);
            thrust::device_vector<double> cuda_delta_hi(parallel_mma.nr_variables(), 0.0);
            ...
            for(size_t iter=0; iter<1; ++iter)
            {
...

                {
                    const double parallel_mma_lb = parallel_mma.lower_bound(); 
                    const double cuda_mma_lb = cuda_mma.lower_bound(); 
                    std::cout << "lb after " << iter << " iterations: parallel mma lb = " << parallel_mma_lb << ", cuda mma lb = " << cuda_mma_lb << "\n";
                    test(std::abs(parallel_mma_lb - cuda_mma_lb) < 1e-6);
                }
            }
        }
