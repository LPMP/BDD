...
void test_mrf(BDD::bdd_mgr & bdd_mgr, bdd_converter & converter)
{
...
void test_simplex(BDD::bdd_mgr & bdd_mgr, bdd_converter & converter)
{
...
void test_cardinality(BDD::bdd_mgr & bdd_mgr, bdd_converter & converter)
{
    std::vector<int> cardinality_weights;
    size_t nr_vars = 100000;
    int rhs = 30;
    for(size_t i=0; i<nr_vars; ++i)
        cardinality_weights.push_back(1);
    // auto & bdd = converter.build_bdd(cardinality_weights.begin(), cardinality_weights.end(), ILP_input::inequality_type::equal, 1);
    // BDD::node_ref bdd2 = bdd.convert_to_lbdd(bdd_mgr);
    lineq_bdd bdd;
    bdd.build_from_inequality(cardinality_weights.begin(), cardinality_weights.end(), ILP_input::inequality_type::equal, rhs);
    std::cout << "Convert to LBDD" << std::endl;
    BDD::node_ref lbdd = bdd.convert_to_lbdd(bdd_mgr);
    std::cout << "# bdd nodes of cardinality constraint with " << nr_vars << " vars " << " and right-and side " << rhs << " : " << lbdd.nodes_postorder().size() << "\n";   
}

void test_miplib(BDD::bdd_mgr & bdd_mgr, bdd_converter & converter)
{
