...
int main(int argc, char** argv)
{
    ...
    std::vector<node_ref> simplex_nodes = simplex.nodes_bfs();

    std::unordered_set<node_ref> node_map;
    for(node_ref x : simplex_nodes)
    {
        test(node_map.count(x.low()) == 0 || x.low().is_terminal(), "nodes not in bfs");
        test(node_map.count(x.high()) == 0 || x.high().is_terminal(), "nodes not in bfs");
        node_map.insert(x);
    } 
}
