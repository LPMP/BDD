...
namespace LPMP {
    ...
    class bdd_lbfgs_cuda_mma {
        public:
            bdd_lbfgs_cuda_mma(BDD::bdd_collection& bdd_col, const int _history_size, 
            ...
            bdd_lbfgs_cuda_mma(BDD::bdd_collection& bdd_col, ITERATOR cost_begin, ITERATOR cost_end,
                const int _history_size, const double _init_step_size = 1e-6, const double _req_rel_lb_increase = 1e-6, 
                const double _step_size_decrease_factor = 0.8, const double _step_size_increase_factor = 1.1,
                const double _gradient_smoothing_factor = 0.0);
            bdd_lbfgs_cuda_mma(bdd_lbfgs_cuda_mma&&);
            bdd_lbfgs_cuda_mma& operator=(bdd_lbfgs_cuda_mma&&);
            ~bdd_lbfgs_cuda_mma();
            template<typename ITERATOR>
    ...
    template<typename REAL>
        template<typename ITERATOR>
        bdd_lbfgs_cuda_mma<REAL>::bdd_lbfgs_cuda_mma(BDD::bdd_collection& bdd_col, ITERATOR cost_begin, ITERATOR cost_end,
        ...
        : bdd_lbfgs_cuda_mma(bdd_col, _history_size,
