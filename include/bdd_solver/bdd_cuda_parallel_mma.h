...
namespace LPMP {
    ...
    class bdd_cuda_parallel_mma : public bdd_cuda_base<REAL> {
        public:
            ...
            bdd_cuda_parallel_mma(const BDD::bdd_collection& bdd_col, const std::vector<double>& costs);
            ...
            void gradient_step(const thrust::device_vector<REAL>& g, double step_size);
    ...
    template<typename REAL>
    void bdd_cuda_parallel_mma<REAL>::gradient_step(const thrust::device_vector<REAL>& g, double step_size)
    {
        gradient_step(g.begin(), g.end(), step_size);
    }
